///////////////////////////////////////////////////////////////////
//
// Служебный модуль с реализацией работы команды ext
//		ExportToFiles
//
///////////////////////////////////////////////////////////////////
#Использовать fs
...
Функция ВыполнитьКоманду(Знач ПараметрыКоманды, Знач ДополнительныеПараметры) Экспорт
	...
	Сообщить("новый старт распаковки "+ТекущаяДата());	
	...
	УK = новый УправлениеКонфигуратором;
	
	УK.УстановитьКонтекст(ЛокальныйКаталогБазы, "", "");
	УK.ВыгрузитьКонфигурациюВФайлы(ЛокальныйКаталогГит+"\src");
	
	
	
	СформироватьФайлКонфигурацииДляВыгрузки(ЛокальныйКаталогБазы, ЛокальныйКаталогГит);
	
	// ФайлыДампа = НайтиФайлы(ЛокальныйКаталогГит, "ConfigDumpInfo.xml", Истина);
	
	// Для каждого файлКУдалению из ФайлыДампа Цикл
	// 	УдалитьФайл(ЛокальныйКаталогБазы+"ConfigDumpInfo.xml");
	// КонецЦикла
...
Процедура СформироватьФайлКонфигурацииДляВыгрузки(ЛокальныйКаталогБазы, ЛокальныйКаталогГит)
	...
	Попытка
		...
		ПараметрыЗапуска.Добавить("/DumpConfigToFiles """+КаталогВыгрузкиФайловКонфигурации+"""");		
		ПараметрыЗапуска.Добавить("/F """+ЛокальныйКаталогБазы+"""");		
		ПараметрыЗапуска.Добавить(" -update");		
		
		Если не ПустаяСтрока(ИмяПользователя1с) Тогда
			ПараметрыЗапуска.Добавить("/N""" +ИмяПользователя1с+"""");		
		КонецЕсли;

		Если  не ПустаяСтрока(ПарольПользователя1с) Тогда			
